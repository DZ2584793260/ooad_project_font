...
export default {
  ...
  methods: {
    ...
    conditionQueryAPI(current, uuid, key) {
      const _this = this
      this.$api.orderApi.GetOrdersNumByConditionWithAccount(this.account, uuid, key, "400")
        .then(res => {
          if (res.data.code == 9000) {
            ...
            _this.$api.orderApi.GetOrdersByConditionWithAccount(_this.pageSize, current, _this.account, uuid, key, "400")
              .then(res => {
    ...
    getAllAPI(size, current) {
      const _this = this
      this.$api.orderApi.GetEvaluatedOrdersNumByUserAccount(this.account)
        .then(res => {
          _this.total = res.data
        }).catch(err => {
          console.log(err);
        });
      this.$api.orderApi.GetEvaluatedOrdersByUserAccount(size, current, this.account)
        .then(res => {
          _this.tableData = res.data
          for (let i = 0; i < _this.tableData.length; i++) {
            _this.tableData[i].produceTime = dayjs(_this.tableData[i].produceTime).format("YYYY-MM-DD HH:mm:ss")
