...
export default {
  ...
  methods: {
    ...
    dialogSave() {
      const _this = this
      this.$api.orderApi.ModifyOrderByUser(this.orderId, dayjs(this.dialogForm.checkInTime).format(), dayjs(this.dialogForm.checkOutTime).format())
        .then(res => {
    ...
    confirmDelete() {
      const _this = this
      this.$api.orderApi.DeleteUnfinishedOrderByUUID(this.tableData[this.row_index].uuid)
        .then(res => {
          this.$message({
            message: "删除成功",
            type: "success"
    ...
    conditionQueryAPI(current, uuid, key) {
      const _this = this
      this.$api.orderApi.GetOrdersNumByConditionWithAccount(this.account, uuid, key, "200")
        .then(res => {
          if (res.data.code == 9000) {
            ...
            _this.$api.orderApi.GetOrdersByConditionWithAccount(_this.pageSize, current, _this.account, uuid, key, "200")
              .then(res => {
    ...
    getAllAPI(size, current) {
      const _this = this
      this.$api.orderApi.GetUnfinishedOrdersNumByUserAccount(this.account)
        .then(res => {
          _this.total = res.data
        }).catch(err => {
          console.log(err);
        });
      this.$api.orderApi.GetUnfinishedOrdersByUserAccount(size, current, this.account)
        .then(res => {
          _this.tableData = res.data
          for (let i = 0; i < _this.tableData.length; i++) {
            _this.tableData[i].produceTime = dayjs(_this.tableData[i].produceTime).format("YYYY-MM-DD HH:mm:ss")
