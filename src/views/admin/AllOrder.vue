            ...
            <el-table :data="tableData" border style="width: 100%"
                ...
                <el-table-column align="center" prop="operation" label="操作" width="200px">
                    <template slot-scope="scope">
                        <el-button type="danger" size="small" round @click="handleDelete(scope.$index)">删除订单</el-button>
                    </template>
                </el-table-column>
...
export default {
    ...
    methods: {
        ...
        conditionQueryAPI(current, uuid, key) {
            const _this = this
            this.$api.orderApi.GetOrdersNumByConditionWithoutAccount(uuid, key, "100")
                .then(res => {
                    if (res.data.code == 9000) {
                        ...
                        _this.$api.orderApi.GetOrdersByConditionWithoutAccount(_this.pageSize, current, uuid, key, "100")
                            .then(res => {
        ...
        getAllAPI(size, current) {
            const _this = this
            this.$api.orderApi.GetAllOrdersNum()
                .then(res => {
                    _this.total = res.data
                }).catch(err => {
                    console.log(err);
                });
            this.$api.orderApi.GetAllOrders(size, current)
                .then(res => {
                    _this.tableData = res.data
                }).catch(err => {
                    console.log(err);
